%%
load("results_3_one_axis_7000gen_8000pop.mat")
...
magnet_conf = [[0;0;0.2],[0.05;0.05;0.2],[-0.05;-0.05;0.2],[0.05;-0.05;0.2],[-0.05;0.05;0.2]];
...
for i = 1:size(sol,1)
    ...
    if magnet_conf_type == "plane"
        ...
        for i = 1:size(magnet_conf,2)
            ...
            J = [];
            for k = 1:sens_num
                one_sensor_jacobian = jacobian_analytical_sensor_reading_to_world(sens_pos(:,k), sens_or_unitary(:,k), magnet_conf(:,i), mu, type);
        ...
        rcond_one_plane_with_magnet_conf = [rcond_one_plane;magnet_conf];

    end
    
    if magnet_conf_type == "cube"
...
%% Plot the plane result with obs as the single objective
x_pos_magnet = rcond_one_plane_with_magnet_conf(2,:);
y_pos_magnet = rcond_one_plane_with_magnet_conf(3,:);
% Reshape the data into a grid
[X, Y] = meshgrid(unique(x_pos_magnet), unique(y_pos_magnet));
Z = griddata(x_pos_magnet, y_pos_magnet, rcond_one_plane_with_magnet_conf(1,:), X, Y);
% Plot the surface
surf(X, Y, Z);
xlabel('Magnet x position');
ylabel('Magnet y position');
