...
% Position = [0.025;0.025;0.15]/scale;
...
% theta_sphere(1) = [];
% 
% [PHI_sphere, THETA_sphere] = ndgrid(phi_sphere, theta_sphere);
% 
% PHI_sphere = reshape(PHI_sphere, [], 1);
% THETA_sphere = reshape(THETA_sphere, [], 1);
% 
% ball = [PHI_sphere, THETA_sphere].';
% 
% ball = [ball, [0;0],[0;pi]];
% 
% Position = [];
...
% Position(3,:) = (Position(3,:) + 0.15)/scale;
...
for i = 1:1
    ...
    for i = 1:length(B_star_collection)
        B_star_corrupted_collection{end+1} = B_star_collection{i} + 10e-7*randn(size(B_star_collection{i}));
    end
