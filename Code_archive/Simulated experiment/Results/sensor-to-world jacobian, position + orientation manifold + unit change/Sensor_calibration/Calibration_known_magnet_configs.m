...
for i = 1:1
    ...
    residual_before = residual_fun(sens_pos, sens_or, P_magnet, Mu_hat_magnet, Volumn, B_r, B_star);
end
...
objective = @(PV_axis) residual_fun(reshape(PV_axis(1:3*num_axis), [3, num_axis]), reshape(PV_axis(3*num_axis+1:end), [3, num_axis]), P_magnet, Mu_hat_magnet, Volumn, B_r, B_star);

% Define the constraint function
nonlcon = @(PV_axis) constraints(PV_axis, num_axis);

% Set optimization options
options = optimoptions('fminimax', 'Display', 'iter', 'MaxFunctionEvaluations',1e5);

% Call fminimax
[PV_opt, fval] = fminimax(objective, PV_axis_0, [], [], [], [], [], [], nonlcon, options);
...
residual_after = residual_fun(P_axis_opt, V_axis_opt, P_magnet, Mu_hat_magnet, Volumn, B_r, B_star);
