...
%% Sensor configuration on the corners of a square, with varying side length
...
for i = 1:rows
    for j = 1:columns
        coordinate_cell{i, j} = [i-1, j-1]; % Store the coordinates as a vector (i-1, j-1)
    end
end
...
for i = 1:rows
    for j = 1:columns
        coords = coordinate_cell{i,j};
        orig_x = coords(1);
        orig_y = coords(2);

        scaled_x = orig_x * (range / (rows-1)) - side_length;
        scaled_y = orig_y * (range / (columns-1)) - side_length;

        coordinate_cell{i,j} = [scaled_x, scaled_y];
    end
end

% Initialize the 2x10000 array
sens_pos = zeros(2, rows * columns);

% Fill the 2x10000 array with the scaled coordinates
index = 1;
for i = 1:rows
    for j = 1:columns
        sens_pos(:, index) = coordinate_cell{i, j};
        index = index + 1;
    end
end
