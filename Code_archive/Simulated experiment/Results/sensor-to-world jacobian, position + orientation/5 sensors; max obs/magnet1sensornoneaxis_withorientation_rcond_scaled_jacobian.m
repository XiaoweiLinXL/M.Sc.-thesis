...
ub = [0.2/scale 0.2/scale 0.0 1 1 1 1 ...
      ...
      0.2/scale 0.2/scale 0.0 1 1 1 1 ...
      1 5];
options = optimoptions(@gamultiobj,'Display','iter', 'MaxStallGenerations', 15000, 'MaxGenerations', 15000, 'PopulationSize', 15000);
...
save('results_5_one_axis_meter_scaledJ_max_rcond_15000gen_15000pop')
%%
% Evaluate
load('results_5_one_axis_meter_scaledJ_max_rcond_10000gen_10000pop.mat')
sens_conf = [sol];
...
% Jacobian of B in sensor frame to magnet position in world frame and
% magnet orientation with respect to world frame, euler angle
function J = jacobian_analytical_sensor_reading_to_magnet_conf_euler_angle_scaled_J(sens_pos, sens_or, magnet_pos, mu_norm, theta, phi, type, Jp_scale) % sens_pos, sens_or, magnet_pos, mu are all in world frame
    mu0 = 4*pi*1e-7;
...
function obj = min_fun_orientation(sens_conf, magnet_conf, mu_norm, type)
    ...
    % Extract the scaling for Jp
    Jp_scale = sens_conf(end);
    sens_conf(end) = [];
...
%     min_magnetic_field_set = [];
    ...
    for magnet_num=1:size(magnet_conf,2)
        ...
        J = [];
        for i=1:sens_num
            J = [J;jacobian_analytical_sensor_reading_to_magnet_conf_euler_angle_scaled_J(sens_pos(:,i), sens_or_unitary(:,i), ...
                magnet_pos, mu_norm, theta, phi, type, Jp_scale)];
        end
...
function [obj_rcond, min_rcond] = evaluate_with_orientation(sens_conf, magnet_conf, mu_norm, type)
    ...
    for num=1:optimal_sol_num
        ...
        for magnet_num=1:size(magnet_conf,2)
            ...
            J_unscaled = [];
            for i=1:sens_num
                J_unscaled = [J_unscaled;jacobian_analytical_sensor_reading_to_magnet_conf_euler_angle(sens_pos(:,i), sens_or_unitary(:,i), ...
                magnet_pos, mu_norm, theta, phi, type)];
            end
    
            sigma = svd(J);
            sigma_unscaled = svd(J_unscaled);
