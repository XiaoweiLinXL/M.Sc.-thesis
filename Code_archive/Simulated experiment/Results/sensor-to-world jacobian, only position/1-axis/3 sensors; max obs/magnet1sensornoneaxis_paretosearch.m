...
options = optimoptions('paretosearch','Display','iter', 'MeshTolerance',1e-7, 'ParetoSetChangeTolerance',1e-10);

fun = @(x) min_fun(x, magnet_conf, mu, type);
[sol, fval, exitflag, output] = paretosearch(fun, length(lb), [], [], [], [], lb, ub, [], options);

save('results_one_axis_paretosearch')

% Evaluate
load('results_one_axis_paretosearch.mat')
sens_conf = [sol];
