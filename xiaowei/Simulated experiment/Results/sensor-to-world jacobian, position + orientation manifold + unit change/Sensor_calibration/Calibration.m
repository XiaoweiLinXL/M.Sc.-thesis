...
P_axis_0 = sens_pos; % initial guess for axis position
V_axis_0 = sens_or; % initial guess for axis orientation

P_magnet_0 = repmat([0;0;0.2], 1, 13);
V_magnet_0 = repmat([-1;0;0], 1, 13);

PV_0 = [P_axis_0(:);V_axis_0(:);P_magnet_0(:);V_magnet_0(:)]; % initial guess

% Objective function is the residual vector
objective = @(PV) residual_fun(reshape(PV(1:3*num_axis), [3, num_axis]), ...
                                    reshape(PV(3*num_axis+1:6*num_axis), [3, num_axis]), ...
                                    reshape(PV(6*num_axis+1:6*num_axis+3*num_timestamp), [3, num_timestamp]), ...
                                    reshape(PV(6*num_axis+3*num_timestamp+1:end), [3, num_timestamp]), ...
                                    Volumn, B_r, B_star);

% Define the constraint function
nonlcon = @(PV) constraints(PV, num_axis, num_timestamp);

% Set optimization options
options = optimoptions('fminimax', 'Display', 'iter', 'MaxFunctionEvaluations',1e6, 'MaxIterations',1e3);

% Call fminimax
[PV_opt, fval] = fminimax(objective, PV_0, [], [], [], [], [], [], nonlcon, options);

% Extract optimized P_axis and V_axis
P_axis_opt = reshape(PV_opt(1:3*num_axis), [3, num_axis]);
V_axis_opt = reshape(PV_opt(3*num_axis+1:6*num_axis), [3, num_axis]);
P_magnet_opt = reshape(PV_opt(6*num_axis+1:6*num_axis+3*num_timestamp), [3, num_timestamp]);
Mu_hat_magnet_opt = reshape(PV_opt(6*num_axis+3*num_timestamp+1:end), [3, num_timestamp]);

%%
residual = residual_fun(P_axis_opt, V_axis_opt, P_magnet_opt, Mu_hat_magnet_opt, Volumn, B_r, B_star);

%% Constraint function
function [c, ceq] = constraints(PV, ncols_V, ncols_M)
    % Extract P_axis, V_axis, P_magnet, and Mu_hat_magnet from PV
    idx_P_axis = 1:3*ncols_V;
    idx_V_axis = 3*ncols_V+1:6*ncols_V;
    idx_P_magnet = 6*ncols_V+1:6*ncols_V+3*ncols_M;
    idx_Mu_hat_magnet = 6*ncols_V+3*ncols_M+1:6*ncols_V+6*ncols_M;
    
    P_axis = reshape(PV(idx_P_axis), [3, ncols_V]);
    V_axis = reshape(PV(idx_V_axis), [3, ncols_V]);
    P_magnet = reshape(PV(idx_P_magnet), [3, ncols_M]);
    Mu_hat_magnet = reshape(PV(idx_Mu_hat_magnet), [3, ncols_M]);

    % Equality constraints for V_axis and Mu_hat_magnet norms
    ceq_V = zeros(1, ncols_V);
    for i = 1:ncols_V
        ceq_V(i) = norm(V_axis(:, i)) - 1;
    end
