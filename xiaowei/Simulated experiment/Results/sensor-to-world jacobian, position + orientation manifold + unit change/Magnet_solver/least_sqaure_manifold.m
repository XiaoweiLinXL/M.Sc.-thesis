...
theta_sphere(1) = [];
theta_sphere(end) = [];
theta = linspace(-pi/2+deg2rad(30),pi/2-deg2rad(30),9);
phi = linspace(-pi/2+deg2rad(30),pi/2-deg2rad(30),9);
...
for i = 1:length(B_star_collection)
    B_star_corrupted_collection{end+1} = B_star_collection{i} + 10e-8*randn(size(B_star_collection{i}));
end
