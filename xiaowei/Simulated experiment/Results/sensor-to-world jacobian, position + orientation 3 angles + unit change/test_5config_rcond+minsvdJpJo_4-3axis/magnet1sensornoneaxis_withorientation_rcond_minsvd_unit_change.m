...
sens_conf = [sol];
[obj_rcond, min_rcond, obj_svd, min_svd, obj_svd_Jp, min_svd_Jp, obj_svd_Jo, min_svd_Jo] = evaluate_with_orientation(sens_conf, magnet_conf, B_r, Volumn, type);
sol
obj_rcond
min_rcond
obj_svd
...
%%
figure
scatter(min_svd_Jp, min_svd_Jo)
xlabel('Minimum singular value of Jp')
ylabel('Minimum singular value of Jo')
title('Minimum singular values of Jp and Jo')

%%
figure
scatter3(min_svd_Jp, min_svd_Jo, min_rcond)
xlabel('Minimum singular value of Jp')
ylabel('Minimum singular value of Jo')
zlabel('Reciprocal condition number')
title('Rcond vs minimum singular values of Jp and Jo')
%% Jacobian test
...
function obj = min_fun_orientation(sens_conf, magnet_conf, B_r, Volumn, type)
    ...
    % the list
    obj = [-min(rcond_set),-min(min_singular_value_Jp_set),-min(min_singular_value_Jo_set)];
    
%     % Maximize all the reciprocal condition number
%     obj = [];
%     for i = 1:size(rcond_set,1)
%         obj = [obj, -rcond_set(i), -min_singular_value_Jp_set(i), -min_singular_value_Jo_set(i)];
%     end
...
%% Evaluation function
function [obj_rcond, min_rcond, obj_svd, min_svd, obj_svd_Jp, min_svd_Jp, obj_svd_Jo, min_svd_Jo] = evaluate_with_orientation(sens_conf, magnet_conf, B_r, Volumn, type)
    ...
    min_svd = min(obj_svd);
    min_svd_Jp = min(obj_svd_Jp);
    min_svd_Jo = min(obj_svd_Jo);
end