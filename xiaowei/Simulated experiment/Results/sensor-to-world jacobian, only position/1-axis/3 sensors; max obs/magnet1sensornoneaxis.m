...
ub = [0.2 0.2 0.0 1 1 1 1 ...
      ...
      0.2 0.2 0.0 1 1 1 1 ...
      3];
options = optimoptions(@gamultiobj,'Display','iter', 'MaxStallGeneration',7000, 'PopulationSize', 8000, 'MaxGenerations', 10000);

fun = @(x) min_fun(x, magnet_conf, mu, type);
[sol, fval, exitflag, output] = gamultiobj(fun, length(lb), [], [], [], [], lb, ub, [], length(lb), options);

save('results_3_three_axis')
%%
% Evaluate
...
find(mean_rcond == max(mean_rcond))
find(min_rcond == max(min_rcond))
